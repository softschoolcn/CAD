								...
								int S_box[8][4][16] = {
									...
									int PP[32] = {
										...
										void TransformData(AcArray<byte>& dats,BOOL bEnc)
										{
											...
											Generate_K(sk);
											ASSERT(dats.logicalLength()!=0);
											//将实体的所有数据存储到文件中
											int nMax_Char_Count=8;//一个字符串最多存储的字符数
											int nLen=dats.logicalLength();
											int nNum=0;
											if(nLen%nMax_Char_Count==0)
											{
												nNum=(int)nLen/nMax_Char_Count;
											}
											else
											{

												nNum=(nLen-nLen%nMax_Char_Count)/nMax_Char_Count+1;
											}
											AcArray<byte> new_dats;
											for(int i=0;i<nNum;i++)
											{
												int nStartIndex=i*nMax_Char_Count;
												int nEndIndex=min((i+1)*nMax_Char_Count,nLen)-1;
												int nSubLen=nEndIndex-nStartIndex+1;
												for(int j=0;j<nSubLen;j++)
												{
													txt[j]=(char)dats[nStartIndex+j];
												}
												for(int j=nSubLen;j<8;j++)
												{
													txt[j]='\0';
												}
												memset(res,0,sizeof(res));
												...
												for(int j=0;j<8;j++)
												{
													new_dats.append((byte)res[j]);
												}
											}
											dats.setLogicalLength(0);
											dats.append(new_dats);
