...
BOOL GRegDialog::OnInitDialog()
{
	...
	if(PathFileExists(m_lpFile))
	{
		...
		if(GetPrivateProfileString(_T("System"),_T("RegKey"),_T(""),lpResult,1024,m_lpFile))
		{
			...
			CString strId1;
			CString strDate;
			CString code(lpResult);
			RegCodeBack(code,strId1,strDate);
			if(strId1.CompareNoCase(strId)==0)
			{
				CString strText;
				strText.Format(_T("您的授权状态为:已注册(%s),感谢您对本软件的支持"),strDate);
...
void GRegDialog::OnBnClickedOk()
{
	...
	CString strId1;
	RegCodeBack(strKey,strId1,strRegDate);
	if(strId.CompareNoCase(strId1)!=0)
	{
		return;
	}
	Glb_bReg=TRUE;
