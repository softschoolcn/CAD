...
class GRIDCTRL_EXT CGridCellBase : public CObject
{
...
public:
	...
	virtual void SetIsMultiLine(BOOL bIsMultiLine)                { m_bIsMultiLine=bIsMultiLine; }
	...
	virtual BOOL IsMultiLine()   const                { return m_bIsMultiLine; }
...
protected:
	...
	void TextOut(CDC* pDC, CString text, CRect* rect, int format);

protected:
    DWORD    m_nState;      // Cell state (selected/focus etc)
	BOOL     m_bIsMultiLine;//是否是多行文本
};
