...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
	...
	m_CellCheckChangeListener=NULL;
                            ...
                            int columnWidth)
{
    ...
    TRY
    {
        ...
        {
            m_arColWidths.InsertAt(nColumn, (UINT)columnWidth);
            if (!GetVirtualMode())
            {
                for (int row = 0; row < m_nRows; row++) 
                {
...
BOOL CGridCtrl::SetColumnWidthPercent(int col, int percent)
{
    if (col < 0 || col >= m_nCols || percent < 0 || percent > 100)
    {
        return FALSE;
    }
    ...
    CRect rect;
    GetClientRect(rect);
    int width = (rect.Width() * percent) / 100;  //º∆À„øÌ∂»÷µ
