...
void CGridCellBase::operator=(const CGridCellBase& cell)
{
	...
	m_bIsMultiLine=cell.m_bIsMultiLine;
}
...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
	...
	//TextOut(pDC, GetText(), &rect, DT_LEFT);
...
void CGridCellBase::TextOut(CDC* pDC, CString text, CRect* drawRect, int format)
{
// 	pDC->SaveDC();
// 	CSize sz;
...
// 				x=text.GetLength();
// 			}
// 			else if (x==text.GetLength()-1) bFind=false;
...
// 			else x++;
// 			s=s+text.Left(x);
// 			text.Delete(0,min(x+1,text.GetLength()));
// 		}
...
// 		case '^':
// 			oldFont = pDC->SelectObject(&SupFont);
// 			pDC->ExtTextOut(pnt.x+supofs.x,pnt.y-supofs.y,ETO_CLIPPED,drawRect,text[0],NULL);
// 			sz = pDC->GetTextExtent(text[0]);
...
// 		case '_':
// 			oldFont = pDC->SelectObject(&SubFont);
// 			pDC->ExtTextOut(pnt.x+subofs.x,pnt.y+subofs.y,ETO_CLIPPED,drawRect,text[0],NULL);
// 			sz = pDC->GetTextExtent(text[0]);
// 			pnt.x+=sz.cx+supofs.x;
// 			text.Delete(0);
// 			pDC->SelectObject(oldFont);
