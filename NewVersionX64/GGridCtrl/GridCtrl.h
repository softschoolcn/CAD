...
//暂时新加上的，无奈之举（后面想办法优化）
struct CGridCellCheckChangeListener
{
	virtual void CellCheckChange(void* pGridCtrl,int row,int col,BOOL bCheck)=0;
};
class CGridCtrl;
...
class GRIDCTRL_EXT CGridCtrl : public CWnd
{
...
public:
    ...
    BOOL SetColumnWidthPercent(int col, int percent);
	...
	CGridCellCheckChangeListener* GetCellCheckChangeListener(){ return m_CellCheckChangeListener;}
	void SetCellCheckChangeListener(CGridCellCheckChangeListener* pListener){m_CellCheckChangeListener=pListener;}
...
public:
    int  InsertColumn(LPCTSTR strHeading, UINT nFormat = DT_CENTER|DT_VCENTER|DT_SINGLELINE,
                      int nColumn = -1, int columnWidth = 0);
...
// Attributes
protected:
	CGridCellCheckChangeListener* m_CellCheckChangeListener;
    // General attributes
