...
CString CByteString::toString(CArray<BYTE>& byteArray)
{
	if(byteArray.GetCount()==0)
		return _T("");
	char* szBuf=new char[2*byteArray.GetCount()+1];
	char char1;
	char char2;
	for(long i=0;i<byteArray.GetCount();i++)
	{
		toString(char1,char2,byteArray[i]);
		szBuf[2*i]=char1;
		szBuf[2*i+1]=char2;
	}
	szBuf[2*byteArray.GetCount()]='\0';
	CString strTmp(szBuf);
	delete[] szBuf;
	szBuf=NULL;
	return strTmp;
