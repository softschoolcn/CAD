                           ...
                           UINT nFirstChar)
{
    ...
    DWORD dwEditStyle = WS_BORDER|WS_CHILD|WS_VISIBLE| ES_AUTOHSCROLL |/*ES_MULTILINE|*/ ES_WANTRETURN | ES_AUTOVSCROLL
        | dwStyle;
    if (!Create(dwEditStyle, rect, pParent, nID)) return;
    
    SetFont(pParent->GetFont());
	CString strText=sInitText;
	if((dwEditStyle&ES_MULTILINE)==ES_MULTILINE)
	{
		//多行文本控件处理
		strText.Replace(_T("\n"),_T("\r\n"));
		strText.Replace(_T("\r\r"),_T("\r"));
	}
