...
void GQrCodeMgr::setQrcodeErrorCorrect(char c)
{
	qrcodeErrorCorrect = c;
}

char GQrCodeMgr::getQrcodeErrorCorrect()
{
	return qrcodeErrorCorrect;
}

int GQrCodeMgr::getQrcodeVersion()
{
	return qrcodeVersion;
}

void GQrCodeMgr::setQrcodeVersion(int i)
{
	if(i >= 0 && i <= 40)
		qrcodeVersion = i;
}

void GQrCodeMgr::setQrcodeEncodeMode(char c)
{
	qrcodeEncodeMode = c;
}

char GQrCodeMgr::getQrcodeEncodeMode()
{
	return qrcodeEncodeMode;
}

void GQrCodeMgr::setStructureappend(int i, int j, int k)
{
...
int GQrCodeMgr::calStructureappendParity(char* abyte0,int i)
{
	int j = 0;
	int k = 0;
	//int i = abyte0.length;
...
bool** GQrCodeMgr::calQrcode(char* abyte0,int iLen0,int& iLenRet)
{
	int j = 0;
	int n1;
	int i = iLen0;
	...
	/*ai1 = (new int[] {
        ...
        try
        {
			...
			CString s;
			s.Format(_T("%s/qrv%d_%d.dat"),GetResRootPath(),qrcodeVersion,byte1);			
        ...
        try
        {
			...
			CString s1;
			s1.Format(_T("%s/qrvfr%d.dat"),GetResRootPath(),qrcodeVersion);
...
char* GQrCodeMgr::divideDataBy8Bits(int* ai, char* abyte0, 
...
char* GQrCodeMgr::calculateRSECC(char* abyte0, char byte0, char* abyte1, int i, int j,
		int iLen0,int iLen1)
    {
	...
	//iLen1 abyte1.length  ; iLen0 abyte0.length
        ...
        {            
			CString s; 
			s.Format(_T("%s/rsc%d.dat") ,GetResRootPath(),byte0);						
    ...
    //private 
char* GQrCodeMgr::calculateByteArrayBits(char* abyte0, char* abyte1, CString s,
...
void GQrCodeMgr::TestMsg(char* myStr,int length) 
{ 
...
char GQrCodeMgr::selectMask(char** abyte0, int i,int iLen0)
{
...
CString GQrCodeMgr::GetResRootPath()
{
	...
	PathRemoveFileSpec(m_lpFile);
	PathAppend(m_lpFile,_T("qrcode_data"));
	return m_lpFile;
}
BYTE* GQrCodeMgr::GenerateBarCode(const char *_pStrInfo, int *_pWidth, int *_pHeight)
{
	if(NULL == _pStrInfo)
	{
		return NULL;
...
bool GQrCodeMgr::SaveBmp(char*bmpName, unsigned char*imgBuf, int width, int height, int biBitCount, RGBQUAD *pColorTable)
{
